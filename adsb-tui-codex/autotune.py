...
def summarize_aircraft(aircraft_json: dict[str, Any]) -> dict[str, Any]:
    ...
    for entry in aircraft:
        ...
        if rssi is not None and rssi >= -20:
            strong_signals += 1
        range_nm = safe_float(entry.get("r_dst"))
        if range_nm is not None:
            max_live_range = max(max_live_range, range_nm)

    return {
        "live_aircraft_count": len(aircraft),
        "live_with_pos": live_with_pos,
        "strong_signals": strong_signals,
        "live_range_nm": max_live_range,
    }
...
def score_snapshot(snapshot: dict[str, Any], config: dict[str, Any]) -> dict[str, Any]:
    ...
    parts = {
        ...
        "live_range_nm": weights["live_range_nm"] * float(live["live_range_nm"]),
        "strong_signals": weights["strong_signals"] * float(live["strong_signals"]),
...
def collect_observation(
    ...
    observation = {
        ...
        "live": {
            "live_range_nm": observation_metric(scored_samples, ("snapshot", "live", "live_range_nm")),
            "strong_signals": observation_metric(scored_samples, ("snapshot", "live", "strong_signals")),
        },
...
def compare_avg(metric: dict[str, float] | None) -> float | None:
...
def compare_max(metric: dict[str, float] | None) -> float | None:
    if not isinstance(metric, dict):
        return None
...
def candidate_acceptance(
    ...
    reasons: list[str] = []
    score_avg = compare_avg(observation.get("score"))
    best_score_avg = compare_avg(best_observation.get("score"))
    ...
    lost_buffers = compare_max(observation.get("airspy", {}).get("lost_buffers"))
    ...
    if minimum_median_snr is not None:
        median_snr = compare_avg(observation.get("airspy", {}).get("median_snr")) or 0.0
        if median_snr < float(minimum_median_snr):
            reasons.append(
                f"median SNR {median_snr:.3f} below minimum {float(minimum_median_snr):.3f}"
            )

    best_aircraft = compare_avg(best_observation.get("readsb", {}).get("aircraft_with_pos")) or 0.0
    current_aircraft = compare_avg(observation.get("readsb", {}).get("aircraft_with_pos")) or 0.0
    ...
    best_msg = compare_avg(best_observation.get("readsb", {}).get("messages_per_second")) or 0.0
    current_msg = compare_avg(observation.get("readsb", {}).get("messages_per_second")) or 0.0
    ...
    best_pos = compare_avg(best_observation.get("readsb", {}).get("positions_per_second")) or 0.0
    current_pos = compare_avg(observation.get("readsb", {}).get("positions_per_second")) or 0.0
    ...
    if minimum_improvement <= score_delta < significant_improvement:
        best_range = compare_avg(best_observation.get("readsb", {}).get("max_range_nm")) or 0.0
        current_range = compare_avg(observation.get("readsb", {}).get("max_range_nm")) or 0.0
        best_live_range = compare_avg(best_observation.get("live", {}).get("live_range_nm")) or 0.0
        current_live_range = compare_avg(observation.get("live", {}).get("live_range_nm")) or 0.0
...
def command_auto_gain_loop(args: argparse.Namespace) -> int:
    ...
    config["baseline_reference"] = {
        "score": compare_avg(baseline_observation.get("score")),
        "median_snr": compare_avg(baseline_observation.get("airspy", {}).get("median_snr")),
    }
    ...
    if args.dry_run:
        report = {
            ...
            "best_score": compare_avg(best_observation.get("score")),
            "history": history,
            "plan": [
                {
                    "candidate": candidate,
        ...
        print(f"Wrote dry-run auto loop plan to {out}")
        return 0
    ...
    for candidate in candidates:
        ...
        try:
            apply_result = apply_extra_env(config, candidate_env_map, reason=f"auto {variable_name} candidate {candidate}")
            ...
            out = write_loop_report(config, f"auto-{variable_name}-loop.latest.json", report)
            raise SystemExit(f"Auto loop aborted on {candidate}; details written to {out}") from exc
    ...
    report = {
        ...
        "env_key": gain_key,
        "baseline_value": baseline_gain,
        "baseline_score": compare_avg(baseline_observation.get("score")),
        "best_value": best_gain,
        "best_score": compare_avg(best_observation.get("score")),
        "history": history,
    }
    out = write_loop_report(config, f"auto-{variable_name}-loop.latest.json", report)
    print(
        f"Auto loop finished for {variable_name}. "
        f"Best value: {best_gain} score={compare_avg(best_observation.get('score'))}"
    )
    print(f"Wrote loop report to {out}")
    return 0
